...
def generate_single_term_index(sentence, lexicon, posting_list, term_id, mode, file_num, constraint_size, forward_index):
    ...
    for words in sentence.values():
        for wordlist in words:
            for word in wordlist.split():
                ...
                # single-term index
                if mode == 1:
                    term_id, file_num = update_lexicon_and_posting_list(lexicon, posting_list, word, doc_no, term_id, file_num, constraint_size, forward_index)
...
def generate_phrase_index(sentence, lexicon, posting_list, term_id, file_num, constraint_num, forward_index):
    stopwords = import_stopwords()
    doc_no = ''.join(sentence.keys())
    for words in sentence.values():
            ...
            if len(temp) <= 1:
                ...
                if temp[0].isalnum() and temp[0] not in stopwords and temp[1].isalnum() and temp[1] not in stopwords:
                    ...
                    term_id, file_num = update_lexicon_and_posting_list(lexicon, posting_list, phrase, doc_no, term_id,
                                                                        file_num, constraint_num, forward_index)
            elif len(temp) > 2:
                for i in range(0, len(temp)):
                    ...
                    if temp[i + 1] not in stopwords and temp[i + 1].isalnum():
                        ...
                        term_id, file_num = update_lexicon_and_posting_list(lexicon, posting_list, phrase, doc_no,
                                                                            term_id, file_num, constraint_num, forward_index)
                        if i < len(temp) - 2:
                            if temp[i + 2].isalnum() and temp[i + 2] not in stopwords:
                                ...
                                term_id, file_num = update_lexicon_and_posting_list(lexicon, posting_list, phrase,
                                                                                    ...
                                                                                    constraint_num, forward_index)
    return term_id, file_num


def update_lexicon_and_posting_list(lexicon, posting_list, term, doc_no, term_id, file_num, constraint_size, forward_index):
    if term in lexicon.keys():
        index = lexicon[term][0]
        lexicon[term][1] += 1
        if doc_no in forward_index.keys():
            if index in forward_index[doc_no].keys():
                forward_index[doc_no][index] += 1
            else:
                forward_index[doc_no][index] = 1
        elif doc_no not in forward_index.keys():
            forward_index.setdefault(doc_no, {index: 1})
        # posting list
        if index in posting_list.keys():
            posting_list[index][1] += 1
        ...
        lexicon.setdefault(term, []).append(term_id)
        lexicon.setdefault(term, []).append(1)
        posting_list.setdefault(term_id, []).append({doc_no: 1})
        posting_list.setdefault(term_id, []).append(1)
        if doc_no in forward_index.keys():
            if term_id in forward_index[doc_no].keys():
                forward_index[doc_no][term_id] += 1
            else:
                forward_index[doc_no][term_id] = 1
        else:
            forward_index.setdefault(doc_no, {term_id: 1})
        term_id = term_id + 1
    return term_id, file_num


def generate_stem_index(sentence, lexicon, posting_list, term_id, file_num, constraint_size, forward_index):
        ...
        for wordlist in words:
            for word in wordlist.split():
                if word.isalnum():
                    ...
                    term_id, file_num = update_lexicon_and_posting_list(lexicon, posting_list, stemmed_word, doc_no, term_id, file_num, constraint_size, forward_index)
    return term_id, file_num
        ...
        for term in lexicon:
            ...
            if mode == 3:
                if posting_list[index][1] >= 6:
                    csv_lexicon.writerow((term, num, len(posting_list[index][0].values())))
                    csv_posting_list.writerow((num, posting_list[index][0], posting_list[index][1], len(posting_list[index][0].values())))
                    num += 1
            elif mode == 2:
                csv_lexicon.writerow((term, index, len(posting_list[index][0].values())))
...
def generate_forward_index(forward_index):
    if os.path.exists("./forward_index.json"):
        os.remove("./forward_index.json")
    with open("./forward_index.json", 'a+', encoding='utf-8') as f:
...
def main():
    ...
    for filename in os.listdir(trec_files_directory_path):
        ...
        for sentence in raw_text:
            if mode == 1 or mode == 2:
                term_id, file_num = generate_single_term_index(sentence, lexicon, posting_list, term_id, mode, file_num, constraint_size, forward_index)
            elif mode == 3:
                term_id, file_num = generate_phrase_index(sentence, lexicon, posting_list, term_id, file_num,
                                                          constraint_size, forward_index)
            elif mode == 4:
                term_id, file_num = generate_stem_index(sentence, lexicon, posting_list, term_id, file_num, constraint_size, forward_index)
    time_2 = generate_output_csv(lexicon, posting_list, mode, file_num, constraint_size, output_dir)
    generate_forward_index(forward_index)
    time_3 = time.time()
    if constraint_size != 0:
